"""Copula-Based Outlier Detection (COPOD) detector."""
...
class COPODDetector(BaseDetector):
    """Simple COPOD-style detector using empirical tail probabilities."""
    ...
    def fit(self, features: object, labels: object | None = None) -> "COPODDetector":
    ...
    def score_samples(self, features: object) -> object:
        ...
        for column_index, column in enumerate(values.T):
            ...
            tail_probability = np.minimum(left_tail, right_tail)
            scores += -np.log(np.maximum(tail_probability, self.tail_smoothing))
